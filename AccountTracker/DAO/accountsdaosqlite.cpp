...
AccountsDAOSQLite::AccountsDAOSQLite(QSqlDatabase& db)
    : _db(db)
{
}
...
Account AccountsDAOSQLite::getAccountByName(const QString& accountName)
{
    ...
    if (query.next()) {
        ...
        return Account(accountId, accountName, sellerName, purchaseDate, invitesCount);
    }
...
Account AccountsDAOSQLite::getAccountById(int id)
{
    ...
    if (query.next()) {
        ...
        return Account(id, accountName, sellerName, purchaseDate, invitesCount);
    }
...
void AccountsDAOSQLite::updateAccount(const Account& account)
{
    ...
    _log.info(__FILE__, "Account updated: " + QString::number(account.getId()));
}
